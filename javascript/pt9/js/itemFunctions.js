...
function castLightning(args) {
    ...
    for (var entity of entities.entities) {
        if (entity.fighter && entity != caster && fovMap.isPointInFov(entity.x, entity.y)) {
...
function castFireball(args) {
    ...
    for (var entity of entities.entities) {
        if (entity.fighter && entity.distanceToPoint(targetX, targetY) <= radius) {
            results.push({"message": new Message("The {0} gets burned for {1} hit points.".format(entity.name, damage))});
            results = results.concat(entity.fighter.takeDamage(damage));
        }
    }
